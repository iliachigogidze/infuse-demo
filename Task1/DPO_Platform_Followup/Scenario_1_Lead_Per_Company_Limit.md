...
How we split depends on what the campaign gives us. The DPO Admin reviews the campaign and **picks one or more split methods to combine**. For example, a campaign could be split by ABM list AND geography, or by geography AND industry. The methods are not mutually exclusive. See **Rules** below.
...
In Rules 1-4 (and their combinations), the counter is a safety net since the split already prevents most collisions. In Rule 5, the counter is the primary mechanism.
...
### Rule 1 - ABM List Split

The campaign's ABM list is divided into exclusive chunks. Each vendor gets their own set of companies. From their perspective, that slice *is* the campaign. Because each company is assigned to exactly one vendor at a time, collisions are prevented structurally. Can be combined with Rules 2-4.
...
### Rule 2 - Geography Split

If the campaign covers multiple regions (e.g., UK vs Ireland, or city-level), assign each vendor a geographic territory. A company's HQ location determines which vendor owns it. Counter runs underneath. Can be combined with other split methods.
...
### Rule 3 - Industry Split

If the campaign targets multiple industries, assign each vendor one or more industry verticals. A company belongs to one industry, so no overlap. Counter runs underneath. Can be combined with other split methods.
...
### Rule 4 - Size Band Split

If the campaign covers a wide company size range, assign each vendor a size band (e.g., enterprise 5000+, mid-market 500-5000). Employee count or revenue defines the boundary. Counter runs underneath. Can be combined with other split methods.
...
The admin can combine these methods. For example, split by ABM list and geography together for a tighter scope. Or split by geography and industry if the campaign spans multiple regions and verticals. The more dimensions combined, the more precise each vendor's scope becomes.

In all cases, the vendor experience is the same: they have a clear scope, they know the cap, and they manage their own work. From the vendor's chair it all feels identical: "here's my territory, here's my limit, go." The counter runs silently underneath as a safety net but rarely triggers because the split prevents collisions structurally.

---

### Rule 5 - No usable split dimension: counter only

When no split method applies (campaign is fully broad, no ABM list), there's nothing to split on. Vendors go find companies on their own. The slot counter is the only enforcement mechanism.
...
Review the campaign dimensions and pick one or more split methods to combine:

**ABM List Split (Rule 1):** Is there an ABM list? If yes, divide it into exclusive vendor chunks. The split can be even or weighted by vendor capacity or geo sub-goals. Load each vendor's chunk into the platform so they only see their assigned companies.

**Geography Split (Rule 2):** Does the GEO have sub-regions? If yes, assign each vendor a geographic territory. Example: campaign says "UK & Ireland" with sub-goals UK 124, Ireland 23. Give Ireland to one vendor, split UK by region between the other two.

**Industry Split (Rule 3):** Does the campaign target multiple industries? If yes, assign each vendor one or more industry verticals.

**Size Band Split (Rule 4):** Is the company size range wide enough to split? If yes, define size bands (e.g., 5000+, 1000-5000, 500-1000) and assign each vendor a band.

These methods can be combined. For example, split by ABM list and geography, or by geography and industry. The more dimensions combined, the tighter each vendor's scope.

**If no split is possible (Rule 5):** Accept the collision risk. Assign vendors to the campaign without splitting. The counter is the only enforcement. Move to Step 3.

### Step 3 - Configure the platform

- Set the company limit on the campaign (if applicable) so the slot counter is active.
- Load vendor-specific company lists (if ABM split used).
- Configure vendor scope restrictions for each split method used (territory, vertical, size band).
- If Rule 5 (no split): no scope restrictions. Vendors work freely.
...
- Their targeting scope (combined from the split methods used: company list, geo territory, industry vertical, size band, or "open" for Rule 5).
